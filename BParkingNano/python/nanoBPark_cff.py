...
from PhysicsTools.BParkingNano.LambdaBToLambdaCMu_cff import *

nanoSequenceOnlyFullSim = cms.Sequence(triggerObjectBParkTables + l1bits)

nanoSequence = cms.Sequence(nanoMetadata + vertexSequence + globalTables +
                            vertexTables + triggerObjectBParkTables + l1bits)

nanoSequenceMC = cms.Sequence(particleLevelBParkSequence +
                              genParticleBParkSequence + globalTablesMC +
                              genWeightsTable + genParticleBParkTables +
                              lheInfoTable)


def nanoAOD_customizeMuonTriggerBPark(process):
    process.nanoSequence = cms.Sequence(
        process.nanoSequence + muonBParkSequence + muonBParkTables
    )  #+ muonTriggerMatchedTables)   ###comment in this extra table in case you want to create the TriggerMuon collection again.
    return process
...
#     process.nanoBKeeSequence = cms.Sequence(electronsBParkSequence +
#                                             electronBParkTables)
#     process.nanoBKstarEESequence = cms.Sequence(electronsBParkSequence +
#                                                 electronBParkTables)
#     return process
...
#     process.nanoBKeeSequence = cms.Sequence(process.nanoBKeeSequence +
#                                             BToKEESequence + BToKeeTable)
...
#     process.nanoBKstarLLSequence = cms.Sequence(KstarToKPiSequence +
#                                                 BToKstarLLSequence +
#                                                 KstarToKPiTable +
#                                                 BToKstarLLTables)
#     return process
...
#     process.nanoBKstarEESequence = cms.Sequence(process.nanoBKstarEESequence +
#                                                 BToKstarEESequence +
#                                                 BToKstarEETable +
#                                                 KstarToKPiTable)
#     return process
...
#     process.nanoBKstarMuMuSequence = cms.Sequence(BToKstarMuMuSequence +
#                                                   BToKstarMuMuTable +
#                                                   KstarToKPiTable)
#     return process


def nanoAOD_customizeLambdaBToLambdaCMu(process):
    process.nanoLambdaBToLambdaCMuSequence = cms.Sequence(
        LambdaBToLambdaCMuSequence + LambdaBToLambdaCMuTable +
        LambdaCToProtonKPiTable)
    return process
