...
options.register('globalTag', 'NOTSET', VarParsing.multiplicity.singleton,
                 VarParsing.varType.string, "Set global tag")
options.register('wantSummary', True, VarParsing.multiplicity.singleton,
                 VarParsing.varType.bool, "Run this on real data")
options.register('wantFullRECO', False, VarParsing.multiplicity.singleton,
                 VarParsing.varType.bool, "Run this on real data")
options.register('reportEvery', 10, VarParsing.multiplicity.singleton,
                 VarParsing.varType.int, "report every N events")
options.register('skip', 0, VarParsing.multiplicity.singleton,
                 VarParsing.varType.int, "skip first N events")
...
#     options.inputFiles = ['file:data_sample.root'] if not options.isMC else \
#                          ['/store/cmst3/group/bpark/BToKmumu_1000Events_MINIAOD.root']
if not options.inputFiles:
    #options.inputFiles = ['/store/data/Run2018B/ParkingBPH4/MINIAOD/05May2019-v2/230000/6B5A24B1-0E6E-504B-8331-BD899EB60110.root'] if not options.isMC else \
    options.inputFiles = ['file:data_sample.root'] if not options.isMC else \
                        ['/store/mc/RunIIAutumn18MiniAOD/BsToPhiMuMu_probeilter_SoftQCDnonD_TuneCP5_13TeV-pythia8-evtgen/MINIAODSIM/PUPoissonAve20_BParking_102X_upgrade2018_realistic_v15-v2/60000/02CF7DD2-435D-0446-B7EA-162D13BA1038.root']
...
process.FEVTDEBUGHLToutput = cms.OutputModule(
    "PoolOutputModule",
    dataset=cms.untracked.PSet(dataTier=cms.untracked.string('GEN-SIM-RECO'),
...
process = nanoAOD_customizeLambdaBToLambdaCMu(process)
...
# process.nanoAOD_KstarMuMu_step = cms.Path(process.nanoSequence +
#                                           process.KstarToKPiSequence +
#                                           process.nanoBKstarMuMuSequence +
#                                           CountBToKstarMuMu)

process.nanoAOD_LambdaCMu_step = cms.Path(
    process.nanoSequence + process.LambdaCToProtonKPiSequence +
    process.nanoLambdaBToLambdaCMuSequence + CountLambdaBToLambdaCMu)
...
process.schedule = cms.Schedule(
    ...
    process.nanoAOD_LambdaCMu_step,
...
process.NANOAODoutput.SelectEvents = cms.untracked.PSet(SelectEvents=cms.vstring(
    #    'nanoAOD_KMuMu_step',
    #    'nanoAOD_Kee_step',
    #    'nanoAOD_KstarMuMu_step',
    'nanoAOD_LambdaCMu_step',
