...
public:
  ...
  explicit TrackMerger(const edm::ParameterSet &cfg) : beamSpotSrc_(consumes<reco::BeamSpot>(cfg.getParameter<edm::InputTag>("beamSpot"))),
...
void TrackMerger::produce(edm::StreamID, edm::Event &evt, edm::EventSetup const &stp) const
{
  ...
  */
  ...
  for (unsigned int iTrk = 0; iTrk < totalTracks; ++iTrk)
  {
    ...
    for (const pat::Muon &imutmp : *muons)
    {
      for (unsigned int i = 0; i < imutmp.numberOfSourceCandidatePtrs(); ++i)
      {
        ...
        {
          matchedToMuon = 1;
          if (imutmp.isLooseMuon())
            matchedToLooseMuon = 1;
          if (imutmp.isSoftMuon(PV))
            matchedToSoftMuon = 1;
          if (imutmp.isMediumMuon())
            matchedToMediumMuon = 1;
          break;
        }
      }
    }
    ...
    if (iTrk < nTracks)
    {
      track = edm::Ptr<pat::PackedCandidate>(tracks, iTrk);
    }
    else
    {
      track = edm::Ptr<pat::PackedCandidate>(lostTracks, iTrk - nTracks);
    }
  ...
  std::sort(vectrk_ttrk.begin(), vectrk_ttrk.end(),
            [](auto &trk1, auto &trk2) -> bool
            { return (trk1.first).pt() > (trk2.first).pt(); });
